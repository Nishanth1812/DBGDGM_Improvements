...
def _download_drive_folder(drive_folder_url: str, download_dir: Path, logger: logging.Logger) -> List[Path]:
    ...
    logger.info(f"Downloading dataset folder from Google Drive: {drive_folder_url}")
    import gdown
...
def _prepare_real_dataloaders_from_drive(
    ...
    """Download/extract/preprocess Google Drive dataset and return dataloaders + holdout metadata."""
    ...
    extract_root = pipeline_root / "extracted"
    ...
    if cache_path.exists():
        logger.info(f"Loading cached preprocessed samples from {cache_path}")
        cached_payload = torch.load(cache_path, map_location="cpu")

        if isinstance(cached_payload, dict) and cached_payload.get('version') == 2:
            ...
            logger.info("Legacy cache format detected; rebuilding cache with inference holdout support")
            samples = []
            inference_series_dirs = []
    ...
    zip_files = _download_drive_folder(drive_folder_url, download_dir, logger)
    ...
    logger.info(f"Preprocessing {len(series_dirs)} DICOM series")
    samples: List[Dict[str, torch.Tensor]] = []
    skipped = 0
    for idx, series_dir in enumerate(series_dirs, start=1):
        ...
        if idx % 50 == 0:
            logger.info(f"Processed {idx}/{len(series_dirs)} series | valid samples={len(samples)}")
    ...
    logger.info(
        f"Saved preprocessed dataset cache to {cache_path} "
        f"(samples={len(samples)}, skipped={skipped}, inference_holdout={len(inference_series_dirs)})"
    )
...
def train(
    ...
    """
    Main training function optimized for Modal.
    
    Args:
        config_path: Path to config YAML (relative to workspace root)
        data_split_info: Optional dict. Use key 'drive_folder_url' for real-data ingestion.
        resume_checkpoint: Path to checkpoint to resume from
        output_prefix: Prefix for output files (checkpoints, logs, etc.)
    ...
    # Load config
    config_full_path = Path(config_path) if Path(config_path).is_absolute() else Path(__file__).parent / config_path
    ...
    use_mock = config['data'].get('mock_data', True)
    ...
    if use_mock:
        ...
        if not drive_folder_url:
            drive_folder_url = config['data'].get('drive_folder_url')

        if not drive_folder_url:
            logger.warning("Real data requested but drive_folder_url is missing. Falling back to mock data.")
            train_loader, val_loader, test_loader = get_adni_dataloaders(
                batch_size=config['data']['batch_size'],
                mock_data=True,
                include_test=True
    ...
    # ========================================================================
    logger.info("Setting up loss and trainer...")
    ...
    annealing_epochs = config['training'].get('kl_annealing_epochs', 20)
...
def predict_unseen_dataset(
    ...
    if series_root:
        ...
        pipeline_root = MOUNT_PATH / "unseen_inference_pipeline"
        download_dir = pipeline_root / "downloads"
        extract_root = pipeline_root / "extracted"
...
def main(
    ...
    """
    print("=" * 80)
    print("MM-DBGDGM Modal Training Launcher")
    print("=" * 80)
    print(f"Config: {config}")
    print(f"Output Prefix: {output_prefix}")
    print(f"Resume From: {resume_from}")
    print(f"Use Mock Data: {use_mock_data}")
    print(f"Drive Folder URL Provided: {bool(drive_folder_url)}")
    print(f"Predict Only: {predict_only}")
    print(f"Predict Dataset: {predict_dataset}")
    print(f"Use Saved Holdout: {use_saved_holdout}")
    print("=" * 80)
    ...
    if predict_dataset:
        ...
        print("\n" + "=" * 80)
        print("Unseen Dataset Inference Result:")
        print("=" * 80)
    ...
    results = train.remote(
        config_path=config,
        data_split_info=None if use_mock_data else {"drive_folder_url": drive_folder_url},
